class Srcom::Api::Games
  ...
  #
  # NOTE: Since `Game`s are very large objects this method defaults to very low results per page
  ...
  def self.find_by(name : String? = nil,
    ...
    return request_games(options, all_pages).map { |raw| Game.from_json(raw.to_json) }
  end
  ...
  def self.find_by_bulk(name : String? = nil,
    ...
    return request_games_bulk(options, all_pages).map { |raw| BulkGame.from_json(raw.to_json) }
  end

  # Finds a `Game` given its *id* or abbreviation.
  def self.get_by_id(id : String) : Srcom::Game
    id = URI.encode(id)
  ...
  def self.get_derived_games(id : String,
                             ...
                             order_by : String? = nil,
                             sort_direction : String? = nil,
                             max_results_per_page : Int32 = 200) : Array(Game)
    options = Hash(String, String).new
    options["name"] = name if name
    options["abbreviation"] = abbreviation if abbreviation
    options["released"] = released.to_s if released
    ...
    return request_derived_games(id, options, all_pages).map { |raw| Game.from_json(raw.to_json) }
  end
  ...
  def self.get_records(id : String,
    ...
    return request_game_records(id, options, all_pages).map { |raw| Leaderboard.from_json(raw.to_json) }
  end

  protected def self.request_games(options : Hash(String, String), all_pages : Bool)
    ...
    return Api.request("/games", url, "GET", all_pages: all_pages)
  end

  protected def self.request_games_bulk(options : Hash(String, String), all_pages : Bool)
    params = URI::Params.encode(options)
    url = "#{BASE_URL}games?_bulk=true&#{params}"

    return Api.request("/games?_bulk=true", url, "GET", all_pages: all_pages)
  end
  ...
  protected def self.request_game_categories(id : String, options : Hash(String, String))
    ...
    return Api.request("/games/#{id}/categories", url, "GET", all_pages: false)
  end
  ...
  protected def self.request_game_levels(id : String, options : Hash(String, String))
    ...
    return Api.request("/games/#{id}/levels", url, "GET", all_pages: false)
  end
  ...
  protected def self.request_game_variables(id : String, options : Hash(String, String))
    ...
    return Api.request("/games/#{id}/variables", url, "GET", all_pages: false)
  end

  protected def self.request_derived_games(id : String, options : Hash(String, String), all_pages : Bool = false)
    params = URI::Params.encode(options)
    ...
    return Api.request("/games/#{id}/derived-games", url, "GET", all_pages: all_pages)
  end

  protected def self.request_game_records(id : String, options : Hash(String, String), all_pages : Bool)
    params = URI::Params.encode(options)
    ...
    return Api.request("/games/#{id}/records", url, "GET", all_pages: all_pages)
  end
end
